...
public class DocumentModel extends ListenerProxy {
	...
	public Page getLinkTargetPage(final int pageDocIndex, final RectF targetRect, final PointF linkPoint, final boolean splitRTL) {
	...
	protected int createFullPage(final IActivityController base, final int docIndex, final int viewIndex, final CodecPageInfo defCpi, final CodecPageInfo info, final PageInfo pi,
			final ArrayList<Page> list) {
	...
	protected int createLeftPage(final IActivityController base, final int docIndex, final int viewIndex, final CodecPageInfo info, final ArrayList<Page> list) {
		final PageIndex index = new PageIndex(docIndex, viewIndex);
	...
	protected int createRightPage(final IActivityController base, final int docIndex, final int viewIndex, final CodecPageInfo info, final ArrayList<Page> list) {
		final PageIndex index = new PageIndex(docIndex, viewIndex);
	...
	private DocumentInfo retrieveDocumentInfo(final IActivityController base, final BookSettings bs, final IProgressIndicator task) {
		...
		for (int i = 0; i < docInfo.docPageCount; i++) {
			if (task != null) {
				task.setProgressDialogMessage(0, (i + 1), docInfo.docPageCount);
			}
