...
public class PageTreeNode implements DecodeService.DecodeCallback {
	...
	public void decodeComplete(final CodecPage codecPage, final ByteBufferBitmap bitmap, final RectF croppedPageBounds) {
		...
		try {
			...
			if (dc instanceof AbstractViewController) {
				EventPool.newEventChildLoaded((AbstractViewController) dc, PageTreeNode.this).process().release();
			}
	...
	class BitmapHolder {
		...
		public boolean drawBitmap(final GLCanvas canvas, final PagePaint paint, final PointF viewBase, final RectF targetRect, final RectF clipRect) {
