...
public final class GLView extends GLRootView implements IView, SurfaceHolder.Callback {
	...
	public GLView showPdf(String path) {
		if (base == null) {
			return null;
		}
		if (base instanceof ViewerActivityController) {
			ViewerActivityController bac = (ViewerActivityController) base;
			bac.setResource(path);
		}
		return this;
	}
	...
	protected final void onLayout(final boolean layoutChanged, final int left, final int top, final int right, final int bottom) {
		...
		final Rect oldLayout = layout.getAndSet(rect);
