...
public class TouchManager {
	...
	public static void setActionEnabled(final String profile, final int id, final boolean enabled, final int left, final int top, final int right, final int bottom) {
		final TouchProfile tp = profiles.get(profile);
		for (final Region r : tp.regions) {
			for (final ActionRef a : r.actions) {
				if (a != null && a.id == id) {
	...
	public static Integer getAction(final Touch type, final float x, final float y, final float width, final float height) {
		return (AppSettings.current().tapsEnabled && stack.peek() != null) ? stack.peek().getAction(type, x, y, width, height) : null;
	}
	...
	public static class Region {
		...
		public RectF getActualRect(final float width, final float height) {
			return new RectF(width * rect.left / 100.0f, height * rect.top / 100.0f, width * rect.right / 100.0f, height * rect.bottom / 100.0f);
		}
		...
		public static Region fromJSON(final JSONObject json) throws JSONException {
			...
			for (int aIndex = 0; aIndex < actions.length(); aIndex++) {
				try {
					...
					throw new JSONException("Old perssitent format found. Touch action are returned to default ones: " + ex.getMessage());
				}
			}
			return region;
		}
