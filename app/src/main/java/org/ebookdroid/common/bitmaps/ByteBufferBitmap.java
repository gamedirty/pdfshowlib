...
public final class ByteBufferBitmap {
	...
	public void applyEffects(final BookSettings bs) {
		final boolean correctContrast = bs.contrast != 100;
		final boolean correctGamma = bs.gamma != 100;
		final boolean correctExposure = bs.exposure != 100;
	...
	public void copyPixelsFrom(final ByteBufferBitmap src, final int left, final int top, final int width, final int height) {
		if (width > this.width) {
	...
	private static native void nativeFillRect(ByteBuffer src, int srcWidth, ByteBuffer dst, int dstWidth, int x, int y, int width, int height);
