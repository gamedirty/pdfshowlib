...
public class Flinger {
	...
	public void fling(final int startX, final int startY, final int velocityX, final int velocityY, final int minX, final int maxX, final int minY, final int maxY) {
		...
		try {
			...
			this.duration = (int) Math.round(FLING_DURATION_PARAM * Math.pow(Math.abs(velocity), 1.0 / (DECELERATED_FACTOR - 1)));

			// System.out.println("Start time:" + startTime + ", duration:" +
			// duration + ", " + startY);
	...
	public boolean computeScrollOffset() {
		...
		try {
			...
			// System.out.println("Flinger.computeScrollOffset(" +
